...
# [환경 설정] - 원본 그대로 유지
MOLIT_API_KEY = "7b6efd99b84e03fca06677a5f9632db682bac3e47d90f5ec37f3b4947e84307e"
...
# [신규 기능] 이원화 파일 자동 생성 함수
def setup_deployment_files():
    print("📁 GitHub Actions 및 배포 설정 파일 생성 중...")
    
    # 1. .github/workflows/update_data.yml 생성
    os.makedirs(".github/workflows", exist_ok=True)
    with open(".github/workflows/update_data.yml", "w", encoding="utf-8") as f:
        f.write(f"""name: RealEstate-Auto-Update
on:
  schedule:
...
jobs:
  build:
    ...
    steps:
      ...
      - run: python main.py auto # 자동 모드로 실행
""")
    ...
    print("✅ 설정 파일 생성 완료: .github, requirements.txt")
...
def create_html():
    ...
    <body>
        ...
        <script>
            ...
            function getFilteredData() {{
                ...
                if(filters.loc) {{ filtered = filtered.filter(i => i.loc_name === filters.loc); }}
                if(filters.period > 0) {{
                ...
                if(filters.price > 0) {{ filtered = filtered.filter(i => Math.abs(i.drop_amount) >= filters.price); }}
            ...
            function render() {{
                ...
                if(filters.loc) {{ status.style.display = 'block'; status.innerText = `📍 ${{filters.loc}} 시그널만 표시 중`; }}
            ...
            function downloadExcel() {{
                const data = getFilteredData().map(i => ({{ "계약일": i.deal_date, "지역": i.loc_name, "단지명": i.apt_name, "실거래가(억)": (i.current_price/10000).toFixed(2), "변동액(억)": (i.drop_amount/10000).toFixed(2) }}));
            ...
            function updateCharts(data) {{
                ...
                charts.ratio = new Chart(document.getElementById('ratioChart'), {{ type: 'doughnut', data: {{ labels: ['상승', '하락'], datasets: [{{ data: [ups, downs], backgroundColor: ['#ef4444', '#3b82f6'], borderWidth:0 }}] }}, options: {{ cutout: '75%' }} }});
                const locCounts = {{}}; 
                ...
                charts.loc = new Chart(document.getElementById('locChart'), {{ type: 'bar', data: {{ labels: Object.keys(locCounts), datasets: [{{ label: '시그널 건수', data: Object.values(locCounts), backgroundColor: '#1e293b', borderRadius:8 }}] }}, options: {{ onClick: (e, el) => {{ if (el.length > 0) {{ filters.loc = Object.keys(locCounts)[el[0].index]; document.getElementById('resetLoc').style.display = 'inline-block'; render(); }} }}, plugins: {{ legend: {{ display: false }} }} }} }});
                const sorted = [...data].sort((a,b) => Math.abs(b.drop_amount) - Math.abs(a.drop_amount)).slice(0, 5);
                if(charts.top) charts.top.destroy();
                charts.top = new Chart(document.getElementById('topChart'), {{ type: 'bar', options: {{ indexAxis: 'y', plugins: {{ legend: {{ display: false }} }} }}, data: {{ labels: sorted.map(i => i.apt_name.substring(0,6)), datasets: [{{ label: '억', data: sorted.map(i => Math.abs(i.drop_amount)/10000), backgroundColor: '#64748b' }}] }} }});
            }}
            init();
        </script>
    </body>
    ...
    # GitHub 서버(Actions)에서는 브라우저를 열지 않도록 처리
...
if __name__ == "__main__":
    ...
    # 2. 실행 모드 판별
    # - GitHub Actions에서 자동으로 돌아갈 때 ('auto' 인자 사용)
    if len(sys.argv) > 1 and sys.argv[1] == "auto":
        print("🤖 자동 모드 실행: 최신 1개월 데이터 수집")
        fetch_data(1) 
    else:
        # - 내 컴퓨터에서 직접 돌릴 때
        choice = input("1.업데이트(3개월)  2.UI확인 : ")
